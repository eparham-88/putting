...
def findFrameBalls_masks_H(frame, frame_balls, index, print_intermediate_frames=False):
    ...
    # frame_out= cv2.warpPerspective(frame, H, size)
    frame_LAB = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    lowers = [[100.0, 16.0,  80.0], [  0.0, 3.0,   80.0], [  0.0, 0.0, 75.0]]
    uppers = [[300.0, 42.0, 100.0], [300.0, 15.0, 100.0], [360.0, 5.0, 100.0]]
    ...
    _, threshold = cv2.threshold(blurred, 10, 255, cv2.THRESH_BINARY)
    ...
    for contour in contours:
        ...
        # find enclosing circle and density
        (x,y),radius = cv2.minEnclosingCircle(contour)
        (x,y),(width,height),angle = cv2.minAreaRect(contour); radius = np.average([width, height]) / 2.0
        pixel_area = cv2.contourArea(contour)
        # density = 100 * float(pixel_area) / float(np.pi*radius**2)
        den = float(width * height)
        if den == 0: den = 1000
        density = 100 * float(pixel_area) / den

        if angle != 90: continue

        if threshold[int(y),int(x)] == 0: continue

        if density < 80: continue

        if pixel_area < 270 or pixel_area > 450: continue
        ...
        cv2.putText(frame_contours, str("density="+"{:.1f}".format(density)),
                    (int(x-45), int(y+radius+50)), cv2.FONT_HERSHEY_DUPLEX, 0.3, (255,0,0))
        cv2.putText(frame_contours, str("area="+"{:.1f}".format(pixel_area)),
                    (int(x-45), int(y+radius+65)), cv2.FONT_HERSHEY_DUPLEX, 0.3, (255,0,0))
...
def findFrameBalls_masks_F(frame, frame_balls, index, print_intermediate_frames=False):
    # Do stuff to frame_out to change the output video
    # frame_out= cv2.warpPerspective(frame, H, size)
    frame_LAB = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    lowers = [[100.0, 16.0,  80.0], [  0.0, 3.0,   80.0], [  0.0, 0.0, 75.0]]
